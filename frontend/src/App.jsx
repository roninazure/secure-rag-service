...
import {ArrowUp, Plus, User, MoreHorizontal, Edit, Copy, RefreshCw, MessageSquare, Sidebar, X, Sun, Moon, Bot, Sparkles, RotateCcw } from 'lucide-react'
...
// Call backend API
    try {
      ...
      console.error('Chat API error details:', err.message, err)
      setMessages(prev => [...prev, {
        id: Date.now() + 1,
        role: 'assistant',
        content: `⚠️ Error: ${err.message}. Check console for details.`,
        timestamp: new Date()
      }])
    } finally {
      setIsTyping(false)
            ...
            <div className="flex items-end gap-3">
              <div className="flex-1 relative">
                ...
                <button
                  ...
                  className={`absolute right-2 bottom-2 z-10 h-10 w-10 rounded-full flex items-center justify-center transition
                    ${input.trim() && !isTyping
                      ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-md hover:shadow-lg transform hover:scale-105'
                      : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 border border-gray-300 dark:border-gray-600 ring-1 ring-gray-300 dark:ring-gray-600'
                    }`}
               >
